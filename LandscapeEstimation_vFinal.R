...
setwd("C:/Users/donnelr3/OneDrive - University Of Cambridge/Archive_J_Cassava/SuperAbundanceLandscape/R")
################################################################################################################
...
################################################################################################################
thefile=paste("legg98central_Scaled_vSouth.csv",sep="") 
...
ugAvrAdValue=colMeans(rbind(ugAdultOnlypre[ugAdultOnlypre$Year==1,1],ugAdultOnlypre[ugAdultOnlypre$Year==2,1],ugAdultOnlypre[ugAdultOnlypre$Year==3,1],ugAdultOnlypre[ugAdultOnlypre$Year==4,1],ugAdultOnlypre[ugAdultOnlypre$Year==5,1]))
ugAvrIncValue=colMeans(rbind(ugIncOnlypre[ugIncOnlypre$Year==1,1],ugIncOnlypre[ugIncOnlypre$Year==2,1],ugIncOnlypre[ugIncOnlypre$Year==3,1],ugIncOnlypre[ugIncOnlypre$Year==4,1],ugIncOnlypre[ugIncOnlypre$Year==5,1]))

avrAbunByInc=ugAvrAdValue/(ugAvrIncValue+1)
temp=ugAdultOnlypre[ugAdultOnlypre$Year==1,c(4,7,8,9,10,11)]
row.names(temp)=seq(1,Dim(temp)[1])
ugAdultAvrOnly=data.frame(temp,avrAbunByInc)
...
regrCubicA <- lm(avrAbunByInc ~ Fieldkm+Field2km+Field3km, data = ugAdultAvrOnlyTrunc)
regrQuadrA <- lm(avrAbunByInc ~ Fieldkm+Field2km, data = ugAdultAvrOnlyTrunc)
regrLinA <- lm(avrAbunByInc ~ Fieldkm, data = ugAdultAvrOnlyTrunc)
...
dat2= list(D_Nfields=DNF_S,
           ...
           D_y=matrix(ugAdultAvrOnlyTrunc$avrAbunByInc, byrow = TRUE, nrow = DL_S, ncol = DNF_S),
           D_subj = adjFields,
           D_intcpt=1   # defaulting to 1, not relevant for the single yr survey data
)
### Run the model and examine results
fitA = stan(file = "rStan/waveProfileModel_v7.stan",
            data = dat2,
            iter = numIter,
            warmup = numWarm,
            chains=numChains,
...
################### 1 # Landscape experiment fitting  Legg et al. 1998 CENTRAL #################################
################################################################################################################
thefile=paste("legg98eastern_Scaled_vSouth.csv",sep="") 
ugData<-read.table(file=thefile, header=TRUE, sep=",")
print(head(ugData))
datasize<-dim(ugData)
...
ugAvrAdValue=colMeans(rbind(ugAdultOnlypre[ugAdultOnlypre$Year==1,1],ugAdultOnlypre[ugAdultOnlypre$Year==2,1],ugAdultOnlypre[ugAdultOnlypre$Year==3,1],ugAdultOnlypre[ugAdultOnlypre$Year==4,1]))
ugAvrIncValue=colMeans(rbind(ugIncOnlypre[ugIncOnlypre$Year==1,1],ugIncOnlypre[ugIncOnlypre$Year==2,1],ugIncOnlypre[ugIncOnlypre$Year==3,1],ugIncOnlypre[ugIncOnlypre$Year==4,1]))
...
# however WILL use likelihood ratio test to justify quadratic model
regrCubicB <- lm(avrAbunByInc ~ Fieldkm+Field2km+Field3km, data = ugAdultAvrOnlyTrunc)
regrQuadrB <- lm(avrAbunByInc ~ Fieldkm+Field2km, data = ugAdultAvrOnlyTrunc)
regrLinB <- lm(avrAbunByInc ~ Fieldkm, data = ugAdultAvrOnlyTrunc)
...
### Run the model and examine results
fitB = stan(file = "rStan/waveProfileModel_v7.stan",
            data = dat2,
            iter = numIter,
            warmup = numWarm,
            chains=numChains,
...
thefile=paste("colvin04_Scaled_vSouth.csv",sep="") 
...
fitC = stan(file = "rStan/waveProfileModel_v7.stan",
            data = dat2,
            iter = numIter,
            warmup = numWarm,
            chains=numChains,
...
summary(fitC, probs = c(0.04, 0.05, 0.95, 0.96))$summary

summary(fitA)$summary
